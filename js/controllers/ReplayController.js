angular.module('app').controller("ReplayCtrl", function($scope,vertxEventBusService,actionData,$stateParams,$state,captureService,pageService,$filter,$sce,$http){
    console.log("entra en replayctrl");
                   ...
                   //  $scope.refreshPage(22);
                    ...
                    $scope.showFilters = function(value){
                     ...
                     $scope.getSnapshots = function(){
                        ...
                        if($scope.SNAPSHOTMethod || $scope.WORKMethod){
                            ...
                            console.log('$scope.GpsqlMethod no está definido: ',$scope.GpsqlMethod);
                    ...
                    /* $scope.getNameFile = function(name){
                     ...
                     $scope.volverCapture = function(){
                        ...
                        if($scope.page==="Database Connect" || $scope.page==="Select the source work" || $scope.page==="Upload file trace"){
                            //captureService.setMethod("salir method");
                             $scope.WORKMethod = false;
                             $scope.SNAPSHOTMethod = false;
							...
							console.log ("comeback select source work"); 
                            $scope.workAllGpsql = false;
                            $scope.gpsqlConnectionOK = false;
							$scope.filters = true;
							$scope.refreshPage(11);
                        }else if($scope.page === "Filters"){
                            if($scope.SNAPSHOTMethod){
                                $scope.selectedOption = true;
                                //$scope.filters = false;
                                $scope.refreshPage(10);
                            }else if($scope.WORKMethod){
                                $scope.refreshPage(11);
                     ...
                     $scope.setMethod = function(method){
                         ...
                         if(method === "WORK"){
							 console.log("Replay method work");
                             $scope.WORKMethod = true;
							 $scope.SNAPSHOTMethod = false;
							 gpsqlConnectionOK = false;
							 $scope.workAllGpsql=true;
							 $scope.refreshPage(11);
                         }
                         if(method === "SNAPSHOT"){
							 console.log("Replay method snapshot");
                             $scope.SNAPSHOTMethod = true;
							 $scope.WORKMethod = false;
							 gpsqlConnectionOK = false;
							 $scope.workAllGpsql=true;
							 $scope.refreshPage(11);
                     ...
                     $scope.replayWork = function(){
                         
                         console.log("función replayWork");
						 vertxEventBusService.send('oracle_replay_actions', {ID_SES:sessionStorage.idSesion, ID_CON: (actionData.setIdCon()).toString(),VERTX_ACTION:"execute_querys_work", ID_WORK_SOURCE:sessionStorage.idWorkOrigen,ID_WORK_DEST:sessionStorage.idWorkDestino,SNAPSHOT_NAME:$scope.snapshotDescription,HOST_NAME:$scope.databaseHostname,PORT:$scope.databasePort,DB_NAME:$scope.databaseName,USER_NAME:$scope.databaseUsername,PASSWD:$scope.databasePassword,ORACLE_SERVICE_SID:$scope.databaseService});
                        console.log('oracle_replay_actions', {ID_SES:sessionStorage.idSesion, ID_CON: (actionData.setIdCon()).toString(),VERTX_ACTION:"execute_querys_work", ID_WORK_SOURCE:sessionStorage.idWorkOrigen,ID_WORK_DEST:sessionStorage.idWorkDestino,SNAPSHOT_NAME:$scope.snapshotDescription,HOST_NAME:$scope.databaseHostname,PORT:$scope.databasePort,DB_NAME:$scope.databaseName,USER_NAME:$scope.databaseUsername,PASSWD:$scope.databasePassword,ORACLE_SERVICE_SID:$scope.databaseService});
						sessionStorage.gohome = true;
                         
                     };
                     
                     $scope.replaySnapshot = function(){        

						console.log("función replaySnapshot");	
						vertxEventBusService.send('oracle_replay_actions', {ID_SES:sessionStorage.idSesion, ID_CON: (actionData.setIdCon()).toString(),VERTX_ACTION:"execute_querys_snapshot", ID_WORK_SOURCE:sessionStorage.idWorkOrigen,ID_WORK_DEST:sessionStorage.idWorkDestino,SNAPSHOT_NAME:$scope.snapshotDescription,ID_SNAPSHOT:sessionStorage.idSnapOrigen.toString(),HOST_NAME:$scope.databaseHostname,PORT:$scope.databasePort,DB_NAME:$scope.databaseName,USER_NAME:$scope.databaseUsername,PASSWD:$scope.databasePassword,ORACLE_SERVICE_SID:$scope.databaseService});
                        console.log('oracle_replay_actions', {ID_SES:sessionStorage.idSesion, ID_CON: (actionData.setIdCon()).toString(),VERTX_ACTION:"execute_querys_snapshot", ID_WORK_SOURCE:sessionStorage.idWorkOrigen,ID_WORK_DEST:sessionStorage.idWorkDestino,SNAPSHOT_NAME:$scope.snapshotDescription,ID_SNAPSHOT:sessionStorage.idSnapOrigen.toString(),HOST_NAME:$scope.databaseHostname,PORT:$scope.databasePort,DB_NAME:$scope.databaseName,USER_NAME:$scope.databaseUsername,PASSWD:$scope.databasePassword,ORACLE_SERVICE_SID:$scope.databaseService});
						sessionStorage.gohome = true;						
